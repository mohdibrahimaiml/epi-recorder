...
Release line observed in source: `v4.1.0`
...
## 2. What v4.1.0 adds over v4.0.3

`v4.1.0` is a security and ecosystem compatibility release. It hardens the trust chain and ensures the toolchain works across modern Python environments. The key improvements are:
...
### 2.2 Typer/Click compatibility hardening
Updated CLI logic to support both `typer >= 0.12.0` and `click >= 8.1.0`, resolving issues where help text or command execution failed in newer environments.

### 2.3 Governance terminology alignment
The viewer UI has been updated to use the term **Governance** instead of **Policy** for user-facing labels (e.g., "Governance Controls", "Governance Basis"). This aligns with enterprise terminology while preserving the underlying "Policy" technical schema.
...
- section 4: governance (policy name, rules, evaluation outcome)
