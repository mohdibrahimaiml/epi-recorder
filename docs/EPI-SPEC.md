# EPI File Format Specification v2.8.7
...
The **Executable Package for AI (EPI)** format provides a standardized, portable, and verifiable container for AI evidence. This specification defines the structure, serialization, and verification mechanisms for `.epi` files as implemented in `epi-recorder` v2.8.7.
...
`v2.8.7`, the canonical layout uses `environment.json`, a root
...
- `v2.8.7` is the current documented layout.
...
| --- | --- | --- | --- |
| **2.8.7** | 2026-03-24 | **Current** | Policy v2 metadata, `tool_permission_guard`, `policy_evaluation.json`, richer control-outcome viewer support, and trust hardening across viewer/installer surfaces. |
