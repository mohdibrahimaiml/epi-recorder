...
function bindEvents() {
  ...
  elements.caseGuidanceReviewButton.addEventListener('click', openCaseReviewForm);
...
async function buildCaseRecord(payload) {
  ...
  const sourceName = payload.sourceName || 'artifact.epi';
  ...
  return {
    ...
    integrity,
    signature,
    trust: deriveTrustState(manifest, integrity, signature),
...
function renderApp() {
  const hasWorkspace = state.cases.length > 0 || Boolean(state.workspaceSetup);
  ...
  if (!hasWorkspace) {
    ...
    state.selectedCaseId = hasCases ? state.cases[0].id : null;
  }
...
function renderSummary() {
  const pendingReviews = state.cases.filter((caseRecord) => caseRecord.reviewState.code === 'pending').length;
...
function updateSetupStorageUi() {
  ...
  elements.setupStorageNote.textContent = hasSaved
    ? `Saved on this device. ${configuredCount ? `${configuredCount} field${configuredCount === 1 ? '' : 's'} are ready for ${SETUP_SYSTEMS[system].label}.` : 'You can reuse this setup any time on this device.'} Secure keys are never saved here.`
    : 'EPI can remember this setup only in this browser on this device. Secure keys stay only in the current session.';
}
...
function formatBridgeHealthLabel(bridgeHealth) {
  ...
  return bridgeHealth.ok ? 'Connected' : 'Not connected';
}
...
function renderSharedWorkspaceStatus() {
  ...
  const label = caseCount ? `${caseCount} team case${caseCount === 1 ? '' : 's'}` : 'Team sync on';
...
async function fetchLiveConnectorRecord() {
  ...
  try {
    const response = await fetch(`${setup.bridgeUrl}/api/fetch-record`, {
      method: 'POST',
      headers: {
    ...
    state.workspaceSetup = setup;
    state.policyEditors[POLICY_EDITOR_EMPTY_KEY] = createPolicyEditorFromSetup(setup);
    const caseRecord = await openLiveConnectorCasePreview(setup, state.liveConnectorRecord);
...
async function openLiveConnectorCasePreview(setup, liveConnectorRecord) {
  const caseRecord = await buildCaseRecord(buildLiveConnectorCasePayload(setup, liveConnectorRecord));
  mergeCases([caseRecord]);
  state.selectedCaseId = caseRecord.id;
  renderApp();
  setView('case');
...
async function refreshSharedWorkspace(showStatus) {
  ...
  try {
    const response = await fetch(`${state.bridgeHealth.url}/api/workspace/state`);
    const payload = await response.json();
    if (!response.ok || !payload.ok) {
...
async function hydrateSharedWorkspaceCases(sharedCases) {
  ...
  for (const item of sharedCases) {
    hydrated.push(await buildCaseRecord({
      id: item.id,
      sourceName: item.source_name || item.sourceName || 'shared-case.epi',
      fileSize: item.file_size || item.fileSize || 0,
      manifest: item.manifest || {},
      steps: item.steps || [],
      analysis: item.analysis || null,
      ...
      sharedUpdatedAt: item.shared_updated_at || item.sharedUpdatedAt || null,
    }));
  }
  mergeCases(hydrated);
}
...
async function publishCaseToSharedWorkspace(caseRecord) {
  ...
  try {
    const response = await fetch(`${state.bridgeHealth.url}/api/workspace/cases`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return `
      <article class="case-card">
        ...
        <div class="case-card-footer">
          <span class="stack-copy">${escapeHtml(caseRecord.risk.detail)}</span>
...
function applySetupWizard() {
  ...
  setView('rules');
  setStatus(
    `Workspace prepared for ${setup.systemLabel} ${setup.workflowLabel.toLowerCase()}. Review the Rules screen, then add case files when you are ready.`,
    'success',
  );
}
...
function buildEmptyInboxContent() {
  if (state.workspaceSetup) {
    return `
      ...
      <p>
        ...
        Review the starting rules now, then try a safe sample, open a live record, or load saved cases.
      </p>
  ...
  return `
    <h3>No cases yet</h3>
    <p>Open one or more saved \`.epi\` files here, use Quick Setup, or start with one example case.</p>
    <div class="action-row empty-state-actions">
      <button class="secondary-button" type="button" data-open-example-case>Open example case</button>
...
function populateReviewForm(caseRecord) {
  ...
  const defaultOutcome = caseRecord.analysis?.primary_fault || caseRecord.analysis?.fault_detected || caseRecord.reviewState.code === 'pending'
    ? 'confirmed_fault'
    : 'dismissed';
  elements.reviewerName.value = existingReview?.reviewer || caseRecord.review?.reviewed_by || '';
...
function buildTimeline(caseRecord) {
  ...
  return visibleSteps.slice(0, 24).map((step, index) => ({
...
function getFilteredCases() {
  return state.cases.filter((caseRecord) => {
    ...
    return matchesSearch && matchesTrust && matchesReview && matchesWorkflow;
  });
}
