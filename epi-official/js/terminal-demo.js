...
class TerminalDemo {
    ...
    init() {
        ...
        this.print([
            { text: 'EPI Portable Environment v4.3.0', color: 'text-dim' },
    ...
    async simulateRecording(filename) {
        const steps = [
            { text: `-> Starting EPI Recorder v4.3.0`, delay: 200 },
    ...
    async simulatePip(args) {
        ...
        if (args[0] === 'install' && args[1] === 'epi-recorder') {
            const lines = [
                ...
                { text: '  Downloading epi_recorder-4.3.0-py3-none-any.whl (28 kB)' },
                { text: 'Installing collected packages: epi-recorder', color: 'text-dim' },
                { text: 'Successfully installed epi-recorder-4.3.0', color: 'text-green' }
            ];
            for (const line of lines) {
                this.print([line]);
